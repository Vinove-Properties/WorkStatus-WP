	...
	private function changeLink($string)
	{
     ...
     private function getTimeAgo($time)
     {
               ...
               /********************* Checking the times and returning correct value */
                if($theminutes < 60){
                 if($theminutes < 1){
                     $timemessage =  esc_html__("Less than 1 minute ago", 'reco');
                 } else if($theminutes == 1) {
                      $timemessage = sprintf( esc_html__("%s minute ago", 'reco'), $theminutes);
                 } else {
                      $timemessage = sprintf( esc_html__("%s minutes ago", 'reco'), $theminutes);
                 }
             } else if($theminutes > 60 && $thedays < 1){
                  if($thehours == 1){
                      $timemessage = sprintf( esc_html__("%s hour ago", 'reco'), $thehours);
                  } else {
                      $timemessage = sprintf( esc_html__("%s hours ago", 'reco'), $thehours);
                  }
             } else {
                  if($thedays == 1){
                      $timemessage = sprintf( esc_html__("%s day ago", 'reco'), $thedays);
                  } else {
                      $timemessage = sprintf( esc_html__("%s days ago", 'reco'), $thedays);
                  }
             }
         return $timemessage;	
     }
