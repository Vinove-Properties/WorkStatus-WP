...
<html lang="en">
  ...
  <body>
            ...
            <section class="pricehead">
              <div class="container">
                <div class="inner-wrap">
                  ...
                  <p>Free 14-day trial. Get two months free when you pay annually.
                  </p>
                </div>
              </div>
            </section>
                                ...
                                <div class="card-body">
                                  <div class="card-body-header">
                                    ...
                                    <a :href="baseurlfrontend + '/auth/register?pid='+ plandata.id+'&type=monthly'"
                                      class="price_btn">{{(plandata.price > 0) ? "Try Free 14 Days Trial" : " Get Started now"}}</a>
                                ...
                                <div class="card-body">
                                  <div class="card-body-header">
                                    ...
                                    <div class="price">
                                      ...
                                      <div v-if="plandata.price_type == 'USD Pricing'"
                                        class="card-plan">
                                        ${{plandata.annual_price > 0 ? parseFloat((plandata.price/12)*10).toFixed(2) : 0}}<span>{{plandata.annual_price > 0 ? " / user / mo" : " forever"}}</span>
                                      </div>
                                      <div v-else class="card-plan">
                                        ₹{{plandata.annual_price > 0 ? parseFloat((plandata.price/12)*10).toFixed(2) : 0}}<span>{{plandata.annual_price > 0 ? " / user / mo" : " forever"}}</span>
                                      </div>
                                      <p class="user_text">Starts at
                                        {{plandata.price_type == 'USD Pricing' ? '$' : '₹'}}{{ plandata.subscription_plan_limitation ? parseFloat(plandata.subscription_plan_limitation.user_number*(parseFloat(((plandata.price/12)*10)*12).toFixed(2))).toFixed(2) : 1*parseFloat((plandata.price/12)*10).toFixed(2) }}
                                        <br v-if="index1 >= 0 ">{{index1 ? " (includes"
                                        : "(includes"}} {{plandata.subscription_plan_limitation ? plandata.subscription_plan_limitation.user_number : 1 }}
                                        {{ (plandata.subscription_plan_limitation  && (plandata.subscription_plan_limitation.user_number > 1)) ? "users)" : "user)"}}
                                      </p>
                                    </div>
                                    <a :href="baseurlfrontend + '/auth/register?pid='+ plandata.id+'&type=yearly'"
                                      class="price_btn">{{(plandata.annual_price > 0) ? "Try Free 14 Days Trial" : " Get Started now"}}</a>
                                    <p class="text_small">No card required, cancel
                                      anytime
                                    </p>
                                  </div>
                    ...
                    <div id="pricecontent">
                      <div id="tab1" class="tab-contents active">
                        <div class="price-cards">
                          <div class="column" v-for="(plan,index) in plans.values">
                            <h3>{{plan.name}}</h3>
                            <p>
                              {{plan.description}}
                            </p>
                            <div v-if="plan.is_custom_support == 0">
                              ...
                              <div class="amount" v-if="plan.price > 0">
                                <strong
                                  v-if="plan.price_type == 'USD Pricing'">${{parseFloat((plan.price/12)*10).toFixed(2)}}</strong>
                                <strong v-else>₹{{parseFloat((plan.price/12)*10).toFixed(2)}}
                                </strong>
                                <sub>/ user / month</sub>
                              </div>
                            ...
                            <div  v-if="plan.price == 0 && plan.is_custom_support==0">
                              <a :href="baseurlfrontend + '/auth/register?pid='+ plan.id+'&type=annual'" class="price_btn">Get Started now</a>
                            </div>
                            <div  v-if="plan.price > 0 && plan.is_custom_support==0">
                              <a :href="baseurlfrontend + '/auth/register?pid='+ plan.id+'&type=annual'" class="price_btn">Try Free 14 Days Trial</a>
                            </div>
                            <div  v-if="plan.price == 0 && plan.is_custom_support==1">
                              <a :href="baseurlfrontend + '/auth/register?pid='+ plan.id+'&type=annual'" class="price_btn">Request a Custom Quote</a>
                            </div>
                              ...
                              <ul class="price">
                                ...
                                <li>
                                  <div  v-if="plan_single.price == 0 && plan_single.is_custom_support==0">
                                    <a :href="baseurlfrontend + '/auth/register?pid='+ plan_single.id+'&type=annual'" class="price_btn">Get Started now</a>
                                  </div>
                                  <div  v-if="plan_single.price > 0 && plan_single.is_custom_support==0">
                                    <a :href="baseurlfrontend + '/auth/register?pid='+ plan_single.id+'&type=annual'" class="price_btn">Try Free 14 Days Trial</a>
                                  </div>
                                  <div  v-if="plan_single.price == 0 && plan_single.is_custom_support==1">
                                    <a :href="baseurlfrontend + '/auth/register?pid='+ plan_single.id+'&type=annual'" class="price_btn">Request a Custom Quote</a>
                                  </div>
                                </li>
                              </ul>
                            </div>
                        ...
                        <div class="price-cards">
                          <div class="column" v-for="(plan_monthly,index_monthly) in plans.values">
                            ...
                            <div  v-if="plan_monthly.price == 0 && plan_monthly.is_custom_support==0">
                              <a :href="baseurlfrontend + '/auth/register?pid='+ plan_monthly.id+'&type=monthly'" class="price_btn">Get Started now</a>
                            </div>
                            <div  v-if="plan_monthly.price > 0 && plan_monthly.is_custom_support==0">
                              <a :href="baseurlfrontend + '/auth/register?pid='+ plan_monthly.id+'&type=monthly'" class="price_btn">Try Free 14 Days Trial</a>
                            </div>
                            <div  v-if="plan_monthly.price == 0 && plan_monthly.is_custom_support==1">
                              <a :href="baseurlfrontend + '/auth/register?pid='+ plan_monthly.id+'&type=monthly'" class="price_btn">Request a Custom Quote</a>
                            </div>
                            <p class="text_small">No card required, cancel anytime</p>
                              ...
                              <ul>
                                <li
                                  ...
                                  <span v-if="features_monthly.values == 'tick'"><i
                                    class="chkicon"></i></span>
                                  <span
                                    v-if="features_monthly.values != 'tick'">{{features_monthly.values}}</i></span>
                                </li>
                              </ul>
                            </div>
                          </div>
                        ...
                        <div class="price-table">
                          <div class="price-row">
                            ...
                            <div class="columns"
                              ...
                              <ul class="price">
                                <li class="price-header">
                                  <h3>{{plan_monthly_single.name}}</h3>
                                ...
                                <li>
                                  <div  v-if="plan_monthly_single.price == 0 && plan_monthly_single.is_custom_support==0">
                                    <a :href="baseurlfrontend + '/auth/register?pid='+ plan_monthly_single.id+'&type=monthly'" class="price_btn">Get Started now</a>
                                  </div>
                                  <div  v-if="plan_monthly_single.price > 0 && plan_monthly_single.is_custom_support==0">
                                    <a :href="baseurlfrontend + '/auth/register?pid='+ plan_monthly_single.id+'&type=monthly'" class="price_btn">Try Free 14 Days Trial</a>
                                  </div>
                                  <div  v-if="plan_monthly_single.price == 0 && plan_monthly_single.is_custom_support==1">
                                    <a :href="baseurlfrontend + '/auth/register?pid='+ plan_monthly_single.id+'&type=monthly'" class="price_btn">Request a Custom Quote</a>
                                  </div>
                                </li>
                              </ul>
                            </div>
    ...
    <script data-cfasync="false" type="text/javascript"
      src="https://cdnjs.cloudflare.com/ajax/libs/vue/3.2.0-beta.3/vue.cjs.js" defer></script>
    ...
    <script data-cfasync="false" src="assests/js/script.js"></script>
    <!-- <script>
      // Define variables
          ...
          var vm = new Vue({
              ...
              data() {
                  return {
                      ...
                      plans: [],
                      params: [],
                      baseurl: 'https://superadmin.staging.workstatus.io/',
